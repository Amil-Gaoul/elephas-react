...
describe('<Upload />', () => {
  ...
  it('should display a file when file is provided', () => {
    const { getAllByTestId } = render(<Upload files={file} label="Выберите файл" onChange={onChange} />);
  ...
  it("shouldn't upload files when appearance is set to 'disabled'", () => {
    const { container } = render(<Upload label="Выберите файл" appearance="disabled" onChange={onChange} />);
  ...
  it('should upload files by click', () => {
    const { getAllByTestId, getByTestId } = render(<Upload label="Выберите файл" onChange={onChange} />);
